...
def print_mcmc_summary(flat_samples, flat_log_prob, param_names, n_data, ndim, n_removed=0):
	"""
...
for aperture in aper_names:
	...
	recommended_burnin, is_adequate = check_burn_in_adequacy(sampler, NBURN, param_names)
	
	# Get samples (discard burn-in, optionally remove outliers)
	if REMOVE_OUTLIER_CHAINS:
		flat_samples, flat_log_prob, n_removed = remove_outlier_chains(sampler, NBURN, n_sigma=OUTLIER_SIGMA)
		# Also apply thinning manually if needed
		if THIN > 1:
			indices = np.arange(0, len(flat_samples), THIN)
			flat_samples = flat_samples[indices]
	...
	# Print summary
	print_mcmc_summary(flat_samples, flat_log_prob, param_names, n_data, ndim, n_removed)

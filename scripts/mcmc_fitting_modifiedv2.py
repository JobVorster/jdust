...
#For BHR71 aper_names = ['b1','o5','b2', 'b3', 'b4','cr1']
aperture_name = 'cr1'
...
grain_species = ['olmg50','pyrmg70','for','ens'] #'qua'
...
# MCMC settings
NWALKERS = 32  # Number of walkers (should be at least 2*ndim)
...
def log_prior(theta):
	...
	theta = [temp, scaling, temp2, scaling2, surface_density, Jv_Scale, mfrac_independent...]
	...
	mfrac_independent = theta[6:]  # Only n-1 fractions
...
def initialize_walkers(p0, nwalkers, ndim, bounds):
	...
	for i in range(nwalkers):
		while True:
			...
			# Independent mass fractions (indices 6 to ndim-1, which is n-1 fractions)
			# Perturb n-1 fractions, ensure they stay positive and sum < 1
			mfrac_independent = np.array(p0[6:])  # n-1 fractions
...
for aperture in aper_names:
	...
	if USE_PREVIOUS_RESULTS:
		...
		if p0_physical_loaded is not None and p0_mfrac_loaded is not None:
			...
			mfrac_flat = mfrac_flat / np.sum(mfrac_flat)  # Ensure normalized
			
			p0_physical = p0_dict[f'ALL:ALL:{model_str}']
			# Only store n-1 mass fractions (last one will be computed as 1 - sum)
	...
	#For some reason these files are unreadable. Gives error about not being able to load the 
	#log_likelihood when pickle.load is called. These files are 3 GB each, and the csv is
	#sufficient for the science at the moment.
	if (0):

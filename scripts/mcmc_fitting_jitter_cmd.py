...
def load_initial_guess_from_csv(csv_foldername, source_name, aperture, n_samples=1000):
	...
	try:
		...
		# Extract jitter if it exists, otherwise use default
		if 'sigma_jitter' in df_tail.columns:
			p0_jitter = df_tail['sigma_jitter'].mean()
		else:
			p0_jitter = 1e-21  # default value
		...
		return p0_physical, p0_jitter, p0_mfrac
...
p0_SD_WIND = 1e-7
p0_FF = 0.5
p0_jitter = 1e-21  # Initial guess for jitter term

p0_dict['ALL:ALL:CMWIND'] = [p0_T1,p0_O1,p0_T2,p0_O2,p0_SD_LOS,p0_O_star,p0_SD_WIND]
p0_dict['ALL:ALL:CM'] =  [p0_T1,p0_O1,p0_T2,p0_O2,p0_SD_LOS,p0_O_star,p0_jitter]  # Added jitter
...
utheta = 180
uFF = 0.5
u_jitter = 1e-18  # Upper bound for jitter
l_jitter = 0      # Lower bound for jitter

#For model CM
bounds_dict['ALL:ALL:CM:T1'] = [0,uT_emit]
bounds_dict['ALL:ALL:CM:O1'] = [0,uO_emit]
bounds_dict['ALL:ALL:CM:SD_LOS'] = [0,uSD]
bounds_dict['ALL:ALL:CM:sigma_jitter'] = [l_jitter, u_jitter]  # Bounds for jitter
...
#	exit()
model_parameters = ['T1','O1','T2','O2','SD_LOS','O_star','sigma_jitter']  # Added sigma_jitter
...
def log_prior(theta):
	...
	temp, scaling, temp2, scaling2, surface_density, Jv_Scale, sigma_jitter = theta[:7]
	...
	# Prior on jitter - log-uniform prior
	if not (_bounds['sigma_jitter'][0] <= sigma_jitter <= _bounds['sigma_jitter'][1]):
		return -np.inf
	else:
		if sigma_jitter > 0:
			log_prob += -np.log(sigma_jitter)
...
def log_likelihood(theta):
	...
	# Include normalization term since we're comparing models with different jitter
...
def initialize_walkers(p0, nwalkers, ndim, bounds):
	...
	for i in range(nwalkers):
		while True:
			...
			# Perturb first 7 physical parameters (including jitter)
			for j in range(7):
...
def print_mcmc_summary(flat_samples, flat_log_prob, param_names_sampled, param_names_all, n_data, ndim, n_removed=0):
	...
	# For chi-squared with jitter, we can compute effective reduced chi-squared
	# Note: The log_prob includes the normalization term, so we need to extract chi2
	# log_prob = -0.5*chi2 + log_norm
	# But we can compute effective chi2_red from the median parameters
	...
	# Extract median jitter to report effective uncertainties
	jitter_idx = param_names_all.index('sigma_jitter')
	median_jitter = np.median(flat_samples[:, jitter_idx])
	print(f"\nJitter parameter:")
...
# Column names for output - added sigma_jitter
columns = ['ID','log_prob','temp','scaling','temp2','scaling2','surface_density','Jv_Scale','sigma_jitter']
for i,gsize in enumerate(grain_sizes):
	for j,gspecies in enumerate(grain_species):
		columns.append(f'{gspecies}-{gsize}um')

# Parameter names - added sigma_jitter
param_names_all = ['temp','scaling','temp2','scaling2','surface_density','Jv_Scale','sigma_jitter']
for i,gsize in enumerate(grain_sizes):
	for j,gspecies in enumerate(grain_species):
...
if USE_PREVIOUS_RESULTS:
	...
	p0_physical_loaded, p0_jitter_loaded, p0_mfrac_loaded = load_initial_guess_from_csv(
	...
	if p0_physical_loaded is not None and p0_mfrac_loaded is not None:
		...
		p0_physical = p0_dict[f'ALL:ALL:{model_str}'][:6]  # First 6 params
		p0_jitter = p0_dict[f'ALL:ALL:{model_str}'][6]     # 7th param is jitter
...
# Combine physical parameters, jitter, and mass fractions
p0 = p0_physical + [p0_jitter] + p0_mfrac
...
# Create DataFrame - now includes log_prob
...
df.to_csv(output_filename, index=False,overwrite=True)
